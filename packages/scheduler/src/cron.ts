...
export function parseCron(expression: string, timezone?: string): CronExpression {
  ...
  return {
    ...
    next(from: Date = new Date()): Date | null {
      ...
      const baseDate = timezone ? getDateInTimezone(from, timezone) : new Date(from);
      ...
      while (iterations < maxIterations) {
        ...
        if (
          minuteExpr && matchesField(minuteExpr, minute, 0, 59) &&
          hourExpr && matchesField(hourExpr, hour, 0, 23) &&
          dayExpr && matchesField(dayExpr, day, 1, 31) &&
          monthExpr && matchesField(monthExpr, month, 1, 12) &&
          dayOfWeekExpr && matchesDayOfWeek(dayOfWeekExpr, dayOfWeek)
        ) {
