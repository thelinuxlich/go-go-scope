...
import { InMemoryJobStorage, Scheduler } from "@go-go-scope/scheduler";
...
async function jobsCommand(scheduler: Scheduler, name: string) {
	...
	printTable(["ID", "Status", "Run At", "Completed", "Retries", "Error"], rows);
...
`);
	...
	try {
		switch (command) {
			...
			case "get":
				if (!name) {
					printError("Schedule name required");
					process.exit(1);
				}
