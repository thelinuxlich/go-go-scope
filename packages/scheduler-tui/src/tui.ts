...
class TUI {
  ...
  constructor(private storage: InMemoryJobStorage) {}
    ...
    process.stdin.on("keypress", (str, key) => {
      ...
      switch (key.name) {
        case "up":
          ...
          break;
        case "down":
          this.selectedIndex = Math.min(this.schedules.length - 1, this.selectedIndex + 1);
          this.draw();
          break;
        case "return":
  ...
  private draw() {
    ...
    // Header
    output += color("cyan", "╔══════════════════════════════════════════════════════════════════════════════╗\n");
    output += color("cyan", "║") + color("bold", " 🚀 go-go-scope Scheduler TUI                                                  ") + color("cyan", "║\n");
    output += color("cyan", "╚══════════════════════════════════════════════════════════════════════════════╝\n");
    ...
    if (this.loading) {
      ...
      output += color("green", "●") + ` ${this.schedules.length} schedules | Last refresh: ${this.lastRefresh.toLocaleTimeString()}`;
    }
    output += color("dim", " | ↑↓ Navigate | Enter: Details | p: Pause | t: Trigger | r: Refresh | q: Quit");
  ...
  private drawList(): string {
    ...
    if (this.schedules.length === 0) {
      output += color("dim", "\n  No schedules found.\n");
      output += color("dim", "  Create one with: npx go-go-scheduler create my-schedule --cron \"0 * * * *\"\n");
      return output;
    }
    ...
    for (let i = 0; i < this.schedules.length; i++) {
      ...
      const stateColor = s.state === ScheduleState.ACTIVE ? "green" : 
                        s.state === ScheduleState.PAUSED ? "yellow" : "red";
      ...
      output += color("dim", ((s.cron || `${s.interval}ms`) as string).padEnd(20)) + " ";
  ...
  private drawDetail(): string {
    ...
    let output = "\n";
    output += color("cyan", "┌─────────────────────────────────────────────────────────────────────────────┐\n");
    output += color("cyan", "│ ") + color("bold", `Schedule: ${s.name}`.padEnd(75)) + color("cyan", "│\n");
    output += color("cyan", "├─────────────────────────────────────────────────────────────────────────────┤\n");

    const stateColor = s.state === ScheduleState.ACTIVE ? "green" : 
                      s.state === ScheduleState.PAUSED ? "yellow" : "red";
    
    output += color("cyan", "│ ") + "State:      " + color(stateColor, s.state.padEnd(65)) + color("cyan", "│\n");
    output += color("cyan", "│ ") + `Schedule:   ${(s.cron || `${s.interval}ms`).padEnd(65)}` + color("cyan", "│\n");
    output += color("cyan", "│ ") + `Timezone:   ${(s.timezone || "default").padEnd(65)}` + color("cyan", "│\n");
    output += color("cyan", "│ ") + `Total Jobs: ${String(s.totalJobs).padEnd(65)}` + color("cyan", "│\n");
    output += color("cyan", "│ ") + "Success:    " + color(s.successRate >= 95 ? "green" : "yellow", `${s.successRate}%`.padEnd(65)) + color("cyan", "│\n");
    
    if (s.averageDuration) {
      output += color("cyan", "│ ") + `Avg Time:   ${`${Math.round(s.averageDuration)}ms`.padEnd(65)}` + color("cyan", "│\n");
    }
    
    output += color("cyan", "│ ") + `Last Run:   ${(s.lastRunAt?.toISOString() || "-").padEnd(65)}` + color("cyan", "│\n");
    output += color("cyan", "│ ") + `Next Run:   ${(s.nextRunAt?.toISOString() || "-").padEnd(65)}` + color("cyan", "│\n");
    output += color("cyan", "├─────────────────────────────────────────────────────────────────────────────┤\n");
    output += color("cyan", "│ ") + "Job Counts:".padEnd(75) + color("cyan", "│\n");
    output += color("cyan", "│ ") + `  Pending:   ${String(s.pendingJobs).padEnd(63)}` + color("cyan", "│\n");
    output += color("cyan", "│ ") + `  Running:   ${String(s.runningJobs).padEnd(63)}` + color("cyan", "│\n");
    output += color("cyan", "│ ") + `  Completed: ${String(s.completedJobs).padEnd(63)}` + color("cyan", "│\n");
    output += color("cyan", "│ ") + `  Failed:    ${String(s.failedJobs).padEnd(63)}` + color("cyan", "│\n");
    output += color("cyan", "│ ") + `  Cancelled: ${String(s.cancelledJobs).padEnd(63)}` + color("cyan", "│\n");
    output += color("cyan", "└─────────────────────────────────────────────────────────────────────────────┘\n");
