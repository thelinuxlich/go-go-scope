...
// Import adapters
let SQLiteAdapter: typeof import("@go-go-scope/persistence-sqlite").SQLiteAdapter;
let BunSQLiteAdapter: typeof import("@go-go-scope/persistence-sqlite-bun").BunSQLiteAdapter;
...
describe("bun compatibility", () => {
  beforeAll(async () => {
    ...
    try {
      SQLiteAdapter = (await import("@go-go-scope/persistence-sqlite")).SQLiteAdapter;
    ...
    // Try bun:sqlite (native Bun module)
    try {
      BunSQLiteAdapter = (await import("@go-go-scope/persistence-sqlite-bun")).BunSQLiteAdapter;
      await import("bun:sqlite");
      availability.bunSqlite = true;
      console.log("✓ bun:sqlite available under Bun");
    } catch (e) {
  ...
  describe.skipIf(!isBun)("bun:sqlite native adapter", () => {
    test("bun:sqlite persistence works", async () => {
      if (!availability.bunSqlite) {
        console.log("Skipping - bun:sqlite not available");
        return;
      }
      ...
      // Test lock
      const lock = await adapter.acquire("bun-native-lock-1", 5000);
      expect(lock).not.toBeNull();
      await lock!.release();

      // Test circuit breaker
      await adapter.recordFailure("bun-native-cb", 3);
      await adapter.recordFailure("bun-native-cb", 3);
      
      let state = await adapter.getState("bun-native-cb");
      expect(state?.failureCount).toBe(2);

      await adapter.recordFailure("bun-native-cb", 3);
      
      state = await adapter.getState("bun-native-cb");
      expect(state?.state).toBe("open");

      await adapter.recordSuccess("bun-native-cb");
  ...
  describe.skipIf(!isBun)("bun native sqlite note", () => {
    test("bun native sqlite has different API", () => {
      ...
      // The BunSQLiteAdapter provides native support for bun:sqlite
