...
describe("memory leak detection", () => {
	...
	test("no memory leak with scheduler job creation and execution", async () => {
		forceGC();
		await new Promise((r) => setTimeout(r, 100));
		const initialMemory = getMemoryMB();

		// Create and execute many scheduled jobs
		for (let i = 0; i < 100; i++) {
			await using s = scope();
			...
			await scheduler.createSchedule("test-schedule", {
				cron: "* * * * *",
				handler: async () => {
			...
			for (let j = 0; j < 10; j++) {
				await scheduler.scheduleJob("test-schedule", { iteration: i, job: j });
			}
	...
	test("no memory leak with scheduler schedules", async () => {
		forceGC();
		await new Promise((r) => setTimeout(r, 100));
		const initialMemory = getMemoryMB();
		...
		for (let i = 0; i < 500; i++) {
			await scheduler.deleteSchedule(`schedule-${i}`);
		}
