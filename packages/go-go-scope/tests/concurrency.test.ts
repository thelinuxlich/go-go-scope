...
describe("stream", () => {
	...
	test("iterates all values", async () => {
		...
		for await (const value of new Stream(testSource(), s)) {
			results.push(value);
		}
	...
	test("stops on abort signal", async () => {
		...
		const iteratePromise = (async () => {
			for await (const value of new Stream(slowSource(), s)) {
				results.push(value);
			}
		})();
	...
	test("cleans up iterator on break", async () => {
		...
		for await (const value of new Stream(sourceWithCleanup(), s)) {
			if (value === 2) break;
		}
	...
	test("scope.stream method works", async () => {
		...
		for await (const value of new Stream(source(), s)) {
			results.push(value);
		}
