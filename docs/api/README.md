...
- **scope** - Creates a new Scope for structured concurrency.
- **parallel** - Runs multiple tasks in parallel with optional concurrency limit and progress tracking.
- **poll** - Polls a function at regular intervals with structured concurrency.
- **race** - Races multiple tasks - the first to settle wins, others are cancelled.

### Core Classes

- **Channel** - A Channel for Go-style concurrent communication between tasks.
- **Scope** - A Scope for structured concurrency.
- **Task** - A disposable task that runs within a Scope.
